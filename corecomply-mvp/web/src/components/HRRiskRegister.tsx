...
export default function HRRiskRegister() {
  ...
  const { data: risks = [], isLoading } = useRisks({ category: 'compliance' });
  const createMutation = useCreateRisk();
  const updateMutation = useUpdateRisk();
  const deleteMutation = useDeleteRisk();

  const canManage = hasPermission('manage_hr_risks');
  const canView = hasPermission('view_hr_risks');
  ...
  const handleCreateSubmit = () => {
    ...
    const createRequest: CreateRiskRequest = {
      ...
      category: formData.category || 'compliance',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        {canManage && (
          <Button onClick={() => setIsAddDialogOpen(true)} data-testid="button-add-hr-risk">
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <Input
            ...
            data-testid="input-search-hr-risks"
          />
        </div>
      </div>
      ...
      {/* View/Create/Edit Dialogs - Same structure as FinanceRiskRegister */}
