...
export default function PayrollRiskRegister() {
  ...
  const { data: risks = [], isLoading } = useRisks({ category: 'operational' });
  const createMutation = useCreateRisk();
  const updateMutation = useUpdateRisk();
  const deleteMutation = useDeleteRisk();

  const canManage = hasPermission('manage_payroll_risks');
  const canView = hasPermission('view_payroll_risks');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        {canManage && (
          <Button onClick={() => setIsAddDialogOpen(true)} data-testid="button-add-payroll-risk">
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <Input
            ...
            data-testid="input-search-payroll-risks"
          />
        </div>
      </div>
      ...
      {/* View/Create/Edit Dialogs - Same structure as other registers */}
